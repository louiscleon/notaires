  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Charger les villes d'intérêt depuis Google Sheets
        const googleSheetsData = await googleSheetsService.loadFromSheet();
        
        // S'assurer que toutes les villes ont des coordonnées
        const villesInteret = await Promise.all(
          googleSheetsData.villesInteret.map(async (ville) => {
            if (!ville.latitude || !ville.longitude) {
              // Géocoder la ville si elle n'a pas de coordonnées
              try {
                const coords = await geocodeAddress(`${ville.nom} ${ville.departement || ''}`);
                ...
                console.error(`Erreur lors du géocodage de ${ville.nom}:`, error);
              }
            }
            return ville;
          })
        ...
        // Mettre à jour les filtres avec les villes d'intérêt géocodées
        setFiltres({
          ...savedData.filtres,
          villesInteret
        });

        // Sauvegarder les villes géocodées dans Google Sheets
        await googleSheetsService.saveVillesInteret(villesInteret);
      } catch (error) {
  ...
  const synchronize = useCallback(async () => {
    ...
    try {
      if (!googleSheetsService.checkConfiguration()) {
        addToast('Configuration Google Sheets manquante. Vérifiez les variables d\'environnement.', 'error');
        return;
      }

      const syncedNotaires = await googleSheetsService.synchronize(notaires);
      setNotaires(syncedNotaires);
